...
def vector_commit(points, scalars):
    return reduce(add, [multiply(P, int(i) % p) for P, i in zip(points, scalars)], Z1) # Mod scalars for safety
...
def commit(a, sL, b, sR, alpha, beta, gamma, tau_1, tau_2):
    ...
    V = add(multiply(G[0], int(np.inner(a, b)) % p), multiply(B, gamma))
    T1 = add(multiply(G[0], (int(np.inner(a, sR)) + int(np.inner(b, sL))) % p), multiply(B, tau_1))
    T2 = add(multiply(G[0], int(np.inner(sL, sR)) % p), multiply(B, tau_2))
    return (A, S, V, T1, T2)
def evaluate(f_0, f_1, f_2, u):
    return (f_0 + f_1 * u + f_2 * u**2) % p
def prove(blinding_0, blinding_1, blinding_2, u):
    pi = evaluate(blinding_0, blinding_1, blinding_2, u)
    return pi
...
sR = np.array([random_element() for _ in range(len(a))])
t1 = (int(np.inner(a, sR)) + int(np.inner(b, sL))) % p
